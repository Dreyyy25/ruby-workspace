...
#3. Write a program that would print the numbers from 0 to 255 but this time, it would also print the sum of the numbers that have been printed so far. For example, your output should be something like this:
#New number: 0 Sum: 0
#New number: 1 Sum: 1
#New number: 2 Sum: 3
#New number: 3 Sum: 6
...
def print_average arr
  return puts "Array is empty" if arr.empty?

  sum = 0
  arr.each { |num| sum += num }

  # Convert sum to float (.to_f) to ensure accurate decimal averages
...
def max_min_avg x 
...

    {
        max: x.max,
        min: x.min,
        avg: x.sum.to_f / x.length
    }
end
p max_min_avg([1, 5, 10, -2])
